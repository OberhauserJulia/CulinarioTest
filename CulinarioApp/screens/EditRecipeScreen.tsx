...
import type { HomeStackParamList } from '../components/navigation/CombinedNavigator';
...
export default function EditRecipeScreen() {
    ...
    const navigation = useNavigation<StackNavigationProp<HomeStackParamList, 'EditRecipe'>>();
    const route = useRoute<RouteProp<HomeStackParamList, 'EditRecipe'>>();
    ...
    // Rezeptdaten laden, wenn Screen fokussiert wird und recipeId existiert
    useFocusEffect(
        React.useCallback(() => {
            if (!recipeId) return;
            let isActive = true;
            async function fetchRecipe() {
                ...
                if (recipe && isActive) {
                    ...
                    setOvenSetting(recipe.ovensettings || "");
                    setSource(recipe.source || "");
                    setImage(recipe.image || null);
                    setIngredientsList(
                        (recipe.ingredients || []).map((ing, idx) => ({
                            input: `${ing.amount || ''} ${ing.name}`.trim(),
                            amount: ing.amount ? String(ing.amount) : '',
                            ingredient: ing.name,
                            foundIngredient: ing.image ? { id: idx, name: ing.name, image: ing.image } : null,
                            isEditing: false,
                            uniqueKey: `edit-${idx}`
                        }))
                            .concat([{ input: '', amount: '', ingredient: '', foundIngredient: null, isEditing: true, uniqueKey: `init-${Date.now()}` }])
                    );
                    setSteps(
                        (recipe.preparationSteps || []).map((step, idx) => ({
                            ...
                            .concat([{ text: '', stepNumber: (recipe.preparationSteps?.length || 0) + 1 }])
                    );
                    setStepIngredients(
    ...
    // Alle Eingaben nur zurücksetzen, wenn KEIN recipeId (also beim neuen Rezept)
    useFocusEffect(
        React.useCallback(() => {
            if (!recipeId) {
                setImage(null);
    ...
    const uploadImageToCloudinary = async (imageUri: string, webAssetObj?: any) => {
        ...
        if (Platform.OS === 'web' && webAssetObj) {
            // Web: File-Objekt direkt anhängen (webAssetObj sollte das File sein)
            ...
            throw new Error('Kein Bild-Upload möglich: Kein File-Objekt für Web gefunden.');
        }
    ...
    async function handleSaveRecipe() {
        ...
        try {
            if (savedImageUri) {
                ...
                // Prüfen, ob das Bild bereits eine Cloudinary-URL ist (bereits hochgeladen)
                if (typeof image === 'string' && (image.startsWith('http://') || image.startsWith('https://'))) {
                    imageUrl = image;
                } else {
                    // wie in AddRecipeScreen: webAsset?.file || webAsset
            ...
            if (!imageUrl || (typeof imageUrl !== 'string' && typeof imageUrl !== 'number')) {
